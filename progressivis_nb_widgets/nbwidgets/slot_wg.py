...
class SlotWg(DataTable):
    ...
    async def refresh(self):
        ...
        if not self.columns:
            ...
            await update_widget(self, 'dt_id', "dt_"+self.module.name+"_"+self.slot_name)
        ...
        if isinstance(tbl, dict):
            #self.data = _dmp({'draw':1,'recordsTotal': 1,'recordsFiltered': 1,
            #             'data': [[0]+list(tbl.values())]})
            await update_widget(self, 'data',  _dmp({'draw':1,'recordsTotal': 1,'recordsFiltered': 1,
                         'data': [[0]+list(tbl.values())]}))
        else: #table
            _len = len(tbl)
            size = 10
            ...
            #                  'recordsFiltered': _len, 'length': _len, 'draw':draw})
            await update_widget(self, 'data', _dmp({'data': data, 'recordsTotal': _len, 
                              'recordsFiltered': _len, 'length': _len, 'draw':draw}))
