...
class _DataClass(object):
    def __init__(
        self, name, that, x_column, y_column, approximate=False, scheduler=None, **kwds
    ):
        self.name = name
        self._group = that.group
        ...
        self.heatmap = None
        self.min_value = that.min_value
        self.max_value = that.max_value
        self.sample = None
    ...
    def create_dependent_modules(
        self, input_module, input_slot, histogram2d=None, heatmap=None, **kwds
    ):
        ...
        with scheduler:
            ...
            if histogram2d is None:
                histogram2d = MCHistogram2D(
                    self.x_column, self.y_column, group=self._group, scheduler=scheduler
                )
...
class MCScatterPlot(NAry):
    ...
    def __init__(self, classes, x_label="x", y_label="y", approximate=False, **kwds):
        ...
        assert classes and isinstance(classes, list)
        if isinstance(classes[0], tuple):
            syn_x, syn_y = zip(*[(x, y) for (_, x, y) in classes])
        else:
    ...
    def make_json(self, json):
        ...
        xbins, ybins = (
            self.hist_tensor.shape[:-1]
            if self._ipydata
            else buffers[0]["binnedPixels"].shape
        )
        encoding = {
            "x": {
                "bin": {"maxbins": xbins},
    ...
    def create_dependent_modules(
        self, input_module=None, input_slot="result", sample="default", **kwds
    ):
        self.input_module = input_module
        self.input_slot = input_slot
        scheduler = self.scheduler()
    ...
    def _finalize(self):
        for dc in self._data_class_dict.values():
            for dc2 in self._data_class_dict.values():
                ...
                dc.histogram2d.input[
                    "table", ("min", x, y)
                ] = dc2.range_query_2d.output.min
                dc.histogram2d.input[
                    "table", ("max", x, y)
                ] = dc2.range_query_2d.output.max
