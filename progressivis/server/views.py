...
def _json_response(*args, **kwargs):
    return web.json_response(*args, dumps=JSONEncoderNp.dumps, **kwargs)
...
async def _scheduler_start(request):
    ...
    if scheduler.is_running():
        return _json_response(
            {"status": "failed", "reason": "scheduler is already running"}
        )
...
async def _scheduler_stop(request):
    ...
    if not scheduler.is_running():
        return _json_response(
            {"status": "failed", "reason": "scheduler is not is_running"}
        )
    scheduler.stop()
    return _json_response({"status": "success"})
...
async def _module(request):
    ...
    if module.is_visualization():
        ...
        return render_template(
            vis + ".html", request, title="%s %s" % (vis, mid), id=mid
        )
...
async def _module_set_parameter(request):
    ...
    try:
        ...
        return _json_response(
            {"status": "failed", "reason": "Cannot set parameters: %s" % execpt}
        )
...
async def _logger_page(request):
    ...
    return render_template(
        "logger.html", request, title="ProgressiVis Loggers", loggers=ret
    )
