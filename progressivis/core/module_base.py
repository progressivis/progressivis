...
ModuleCb = Callable[["BaseModule", int], None]
ModuleCoro = Callable[["BaseModule", int], Coroutine[Any, Any, Any]]
...
class GroupContext:
    ...
    def __init__(self, modgroup: Union[str, BaseModule]) -> None:
...
class ModuleCallbackList(List[ModuleProc]):
    async def fire(self, module: BaseModule, run_number: int) -> bool:
        ret = False
        for proc in self:
            try:
...
class BaseModule(metaclass=ABCMeta):
    ...
    def __new__(cls, *args: Tuple[str, Any], **kwds: Any) -> BaseModule:
    ...
    def __init__(
        ...
        **kwds: Any,
        ...
        self._state: ModuleState = BaseModule.state_created
        self._saved_state: ModuleState = BaseModule.state_invalid
    ...
    def terminate(self) -> None:
        ...
        self.state = BaseModule.state_zombie

    def create_slot(
        self,
        output_name: Union[str, int],
        input_module: Optional[BaseModule],
    ...
    def connect_output(
        self, output_name: str, input_module: BaseModule, input_name: str
    ) -> Slot:
    ...
    def get_input_module(self, name: str) -> Optional[BaseModule]:
    ...
    def validate(self) -> None:
        ...
        if self.state == self.state_created:
            self.state = BaseModule.state_ready
    ...
    def get_data(self, name: str) -> Any:
        if name == BaseModule.TRACE_SLOT:
            return self.tracer.trace_stats()
        if name == BaseModule.PARAMETERS_SLOT:
            return self._params
    ...
    def next_state(slot: Slot) -> ModuleState:
        ...
        if slot.has_buffered():
            return BaseModule.state_ready
        return BaseModule.state_blocked

    def _return_run_step(
        self, next_state: ModuleState, steps_run: int
    ) -> ReturnRunStep:
        assert next_state >= BaseModule.state_ready and next_state <= BaseModule.state_zombie
    ...
    def _return_terminate(self, steps_run: int = 0) -> ReturnRunStep:
        return {"next_state": BaseModule.state_zombie, "steps_run": steps_run}
    ...
    def is_created(self) -> bool:
        return self._state == BaseModule.state_created

    def is_running(self) -> bool:
        return self._state == BaseModule.state_running

    def is_suspended(self) -> bool:
        return self._state == BaseModule.state_suspended

    def suspend(self) -> None:
        if self._state == BaseModule.state_running:
            raise RuntimeError("Cannot suspend a running module (yet)")
        elif self._state == BaseModule.state_suspended:
            return
        self._saved_state = self._state
        self._state = BaseModule.state_suspended

    def resume(self) -> None:
        if self._state != BaseModule.state_suspended:
            raise RuntimeError("Cannot resume a module not suspended")
        self._state = self._saved_state
        self._saved_state = BaseModule.state_invalid

    def prepare_run(self, run_number: int) -> None:
        "Switch from zombie to terminated, or update slots."
        if self.state == BaseModule.state_zombie:
            self.state = BaseModule.state_terminated
            return
    ...
    def is_ready(self) -> bool:
        if self.state == BaseModule.state_terminated:
            logger.info(f"{self.name} Not ready because it terminated")
            return False
        if self.state == BaseModule.state_invalid:
            logger.info(f"{self.name} Not ready because it is invalid")
            return False
        if self.state == BaseModule.state_suspended:
        ...
        for slot in slots:
            ...
            if slot.has_buffered() or in_ts > ts:
                ready_count += 1
            elif in_module.is_terminated() or in_module.state == BaseModule.state_invalid:
                term_count += 1
        ...
        if self.state == BaseModule.state_blocked:
        ...
        if self.state == BaseModule.state_ready:
            return True
    ...
    def is_zombie(self) -> bool:
        return self._state == BaseModule.state_zombie

    def is_terminated(self) -> bool:
        return self._state == BaseModule.state_terminated

    def is_valid(self) -> bool:
        return self._state != BaseModule.state_invalid
    ...
    def set_state(self, s: ModuleState) -> None:
        assert (
            s.value >= BaseModule.state_created.value
            and s.value <= BaseModule.state_invalid.value
    ...
    async def ending(self) -> None:
        ...
        self._state = BaseModule.state_terminated
    ...
    def run(self, run_number: int) -> None:
        ...
        self.state = BaseModule.state_running
        ...
        if step_size != 0:
            ...
            try:
                ...
                logger.info("In BaseModule.run(): Received a StopIteration")
                next_state = BaseModule.state_zombie
            ...
            if self._start_time == 0 or self.state != BaseModule.state_ready:
    ...
    def _path_to_origin_impl(self) -> List[BaseModule]:
        res = [
            item
            for sublist in [
...
class InputSlots:
    ...
    def __dir__(self) -> Iterable[str]:
        module: BaseModule = self.__dict__["module"]
...
def def_parameter(name, type, value):
    """
    class decorator
    """
...
def def_input(name, type=None, **kw):
    """
    class decorator
    """
    def module_decorator(module):
...
def def_output(name, type=None, attr_name=None, custom_attr=False, **kw):
    """
    class decorator
    """
...
class Every(BaseModule):
    ...
    def run_step(
        ...
        self._proc(df)
        slot.clear_buffers()
        return self._return_run_step(BaseModule.state_blocked, steps_run=1)
