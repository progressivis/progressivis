...
class TestProgressiveLoadCSV(ProgressiveTest):
    ...
    def test_read_csv(self):
        s=self.scheduler()
        module=SimpleCSVLoader(get_dataset('bigfile'), index_col=False, header=None, scheduler=s)
    ...
    def test_read_fake_csv(self):
        s=self.scheduler()
        module=SimpleCSVLoader(RandomBytesIO(cols=30, rows=1000000), index_col=False, header=None, scheduler=s)
        self.assertTrue(module.result is None)
        aio.run(s.start())
        self.assertEqual(len(module.result), 1000000)
